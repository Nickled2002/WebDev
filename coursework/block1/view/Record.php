...
<html lang="en">
	...
	<body>
		...
		<div class="jumbotron text-center" style="background-color:black;margin-bottom:0;">
			...
			<div class="card-img-overlay">
            		...
            		<h3>Welcome Admin Transaction Table</h3>
			</div>
        	</div>
			</div>
	...
	<!-- Display items -->
		<?php
			...
			$itemtxt = getAllTransaction() ;
			$item = json_decode($itemtxt) ;
			for ($i=0; $i< sizeof($item); $i++) {
				echo '<div class="col-lg-4 col-md-4 col-sm-4">' ;
				echo '<div class="card">' ;
				echo '<div class="card-header">' ;
				echo '<h3> Transaction Number: '.$item[$i] -> id.'</h3>';
				echo '</div>' ;
				echo '<div class="card-block">' ;
				echo '<p>Product :'.$item[$i] -> itemname.'</p>' ;
				echo '<p>Transaction Id :'.$item[$i] -> transaction_name.'</p>' ;  
				echo '</div>' ;
				echo '<div class="card-footer">' ;
				echo '<p>User Name :'.$item[$i] -> name.'</p>' ;
				echo '<p>User Email :'.$item[$i] -> email.'</p>' ; 
				echo '<p>User Address :'.$item[$i] -> address.'</p>' ;  
				echo '</div>' ;
				echo '</div>' ;
				echo '</div>' ;
			}
			?><!-- echo '<button type="button" class="btn btn-primary">Add to basket</button>'; -->
	</div><!-- row -->
